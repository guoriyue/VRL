...
def test_shutdown_closes_owned_group_once(monkeypatch) -> None:
    calls = []
    strategy = _ddp_strategy(_cpu_ddp_context())
    monkeypatch.setattr(
        "vrl.trainers.strategy.init_training_process_group", lambda *args, **kwargs: True
    )
    monkeypatch.setattr(torch.nn.parallel, "DistributedDataParallel", lambda module, **kw: module)
    monkeypatch.setattr(
        "vrl.trainers.strategy.shutdown_training_process_group", lambda: calls.append("close")
    )
    strategy.prepare_model(FakePolicy(ToyTransformer()))
    strategy.shutdown()
    strategy.shutdown()
...
def test_peer_checkpoint_optimizer_export_is_empty() -> None:
    strategy = _ddp_strategy(
        DistributedTrainingContext(
            strategy="ddp", rank=1, world_size=2, device=torch.device("cpu")
        )
    )
    model = nn.Linear(2, 2)
    optimizer = torch.optim.Adam(model.parameters())
    assert strategy.export_checkpoint_optimizer_state(model, optimizer) == {}
    assert strategy.export_optimizer_state(model, optimizer) == optimizer.state_dict()


def test_group_creation_cleans_up_when_coordination_subgroup_fails(monkeypatch) -> None:
    from vrl.trainers.distributed import init_training_process_group

    calls = []
    monkeypatch.setattr(torch.distributed, "is_initialized", lambda: bool(calls))
    monkeypatch.setattr(torch.distributed, "init_process_group", lambda **kw: calls.append("init"))
    monkeypatch.setattr(
        torch.distributed, "destroy_process_group", lambda: calls.append("destroy")
    )

    def fail_subgroup(**kwargs):
        raise RuntimeError("subgroup failed")

    monkeypatch.setattr(torch.distributed, "new_group", fail_subgroup)
    with pytest.raises(RuntimeError, match="subgroup failed"):
        init_training_process_group(_cpu_ddp_context(), backend="nccl")
