...
def test_run_fixed_eval_uses_fixed_seed_grid_and_no_backward() -> None:
    ...
    class _Collector:
        # Deliberately exposes ONLY collect_unscored/score_rollouts — no
        # collect_training_batch / backward, so eval physically cannot train.
        async def collect_unscored(self, prompts, **kwargs):
            seen_seeds.append(int(kwargs["seed"]))
            return {"group_size": int(kwargs["group_size"])}

        async def score_rollouts(self, unscored):
            return [_Batch(torch.arange(u["group_size"], dtype=torch.float32)) for u in unscored]
    ...
    assert result.reward_mean == pytest.approx(0.5)  # rewards [0,1,0,1]
    assert result.component_means["kling"] == pytest.approx(2.0)

    # Re-running yields the SAME seed grid (reproducible across epochs/resumes).
    seen_seeds.clear()
    _eval()
    assert seen_seeds == [100, 102]
...
def test_eval_config_validation() -> None:
    """EvalConfig is off by default and rejects out-of-range fields."""
    import pytest

    from vrl.trainers.core.types import EvalConfig

    assert EvalConfig().enabled is False
    EvalConfig(enabled=True, freq=1, samples_per_prompt=1, max_prompts=0, seed=0)
    for bad, match in [
        (dict(freq=0), "freq"),
        (dict(samples_per_prompt=0), "samples_per_prompt"),
        (dict(max_prompts=-1), "max_prompts"),
        (dict(seed=-1), "seed"),
    ]:
