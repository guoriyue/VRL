"""Env-gated real checkpoint validation for Ray rollout workers."""
...
def _require_real_ray_validation() -> None:
    if os.environ.get("VRL_RAY_REAL_VALIDATION") != "1":
        pytest.skip("VRL_RAY_REAL_VALIDATION=1 is required for real Ray rollout validation")
...
def _require_cuda_for_workers(num_workers: int):
    ...
    if not torch.cuda.is_available():
        pytest.skip("CUDA is unavailable; real Ray rollout validation cannot run")
    if torch.cuda.device_count() < num_workers:
        pytest.skip(
            f"real Ray rollout validation needs {num_workers} CUDA devices, "
...
@pytest.mark.asyncio
async def test_real_sd3_5_ray_rollout_validation() -> None:
    _require_real_ray_validation()
    ...
    cfg = load_config(
        ...
        overrides=[
            ...
            f"[{','.join(str(idx) for idx in range(num_workers))}]",
            "distributed.resources.trainer.num_gpus=0",
            f"distributed.resources.rollout.num_gpus={num_workers}",
            "distributed.resources.rollout.gpus_per_worker=1",
            f"distributed.resources.rollout.num_workers={num_workers}",
    ...
    try:
        output = await runtime.generate(
            GenerationRequest(
                request_id="real-sd3-ray-validation",
                family="sd3_5",
                task="t2i",
