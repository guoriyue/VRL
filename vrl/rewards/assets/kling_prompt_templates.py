...
def build_kling_video_reward_prompt(
    prompt: str,
    dimension: str | list[str],
    template_type: str,
) -> str:
    """Build one checkpoint-compatible VideoReward evaluation prompt."""

    dimensions = [dimension] if isinstance(dimension, str) else list(dimension)
    if not dimensions:
        raise ValueError("Kling VideoReward eval_dim must be non-empty")
    unknown = [name for name in dimensions if name not in _DIMENSION_DESCRIPTIONS]
    if unknown:
        raise ValueError(
            "unsupported Kling VideoReward dimension(s): "
            f"{unknown}; expected one of {sorted(_DIMENSION_DESCRIPTIONS)}",
        )

    if len(dimensions) > 1:
        dimension_name = ", ".join(_DIMENSION_DESCRIPTIONS[name][0] for name in dimensions)
        dimension_name = f"overall performance({dimension_name})"
        dimension_description = "the overall performance of the video"
    else:
        dimension_name, dimension_description = _DIMENSION_DESCRIPTIONS[dimensions[0]]
...
__all__ = ["build_kling_video_reward_prompt"]
