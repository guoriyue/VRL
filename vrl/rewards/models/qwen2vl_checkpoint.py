...
Published reward checkpoints (HPSv3, Kling VideoReward) predate that rename, so
their keys must be relocated before a ``strict=True`` load into a current
model. The relocation is per key and idempotent, so a state dict that already
mixes both layouts (a live model's keys updated with a legacy LoRA/non-LoRA
split) still lands on the nested layout.
"""
...
__all__ = ["remap_legacy_qwen2vl_key", "remap_legacy_qwen2vl_state_dict"]
