"""OpenPose-style anime anatomy structure reward as a batch-capable RewardModel.

Runs a DWPose ONNX whole-body keypoint detector over a rollout's images and
scores anatomy structure from keypoint geometry. Implements ``score_request`` so
the detector is invoked once across every artifact's images (preserving the
original in-process batching), then per-rollout scores are aggregated by mean.
"""
...
class AnimeAnatomyStructureRewardModel:
    """Batch RewardModel returning ``{"anime_anatomy_structure": value}`` per artifact."""

    def __init__(self, worker_config: Mapping[str, Any]) -> None:
        cfg = dict(worker_config)
        self._model_repo = str(cfg.get("model_repo", "yzd-v/DWPose"))
        self._detector_file = str(cfg.get("detector_file", "yolox_l.onnx"))
        self._pose_file = str(cfg.get("pose_file", "dw-ll_ucoco_384.onnx"))
        self._cache_dir = str(cfg.get("cache_dir", "") or "")
        self._local_files_only = bool(cfg.get("local_files_only", False))
        self._device = str(cfg.get("device", "cuda"))
        self._detector_factory: Callable[[list[Any]], list[Any]] | None = cfg.get("detector")
    ...
    def score_request(self, request: Any) -> list[dict[str, float]]:
        ...
        for group in image_groups:
            ...
            out.append({"anime_anatomy_structure": value})
        return out
    ...
    def _detect(self, flat_images: list[Any]) -> list[Any]:
        ...
        if self._detector_factory is not None:
            return list(self._detector_factory(flat_images))
        ...
        return [self._detector(image) for image in flat_images]
...
def anime_anatomy_structure_reward_model(
    worker_config: Mapping[str, Any],
) -> AnimeAnatomyStructureRewardModel:
    return AnimeAnatomyStructureRewardModel(worker_config)
...
__all__ = [
    "AnimeAnatomyStructureRewardModel",
    "anime_anatomy_structure_reward_model",
]
