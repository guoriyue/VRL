...
# The only released Cosmos3 generator checkpoint (omnimodal, 16B). Inlined here
# rather than read from a model-support table: this probe is its single consumer.
_DEFAULT_MODEL = "nvidia/Cosmos3-Nano"


def _new_report(args: argparse.Namespace) -> dict[str, Any]:
    """Seed the decision note with the open contract questions."""
    return {
...
def main() -> None:
    ...
    parser.add_argument("--model", default=None, help=f"override (default: {_DEFAULT_MODEL})")
    parser.add_argument(
        "--out",
