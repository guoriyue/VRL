...
from vrl.math.diffusion.flow_matching import SDEStepResult
...
def ddim_step_with_logprob(
    ...
    abar_prev_flat = torch.stack(
        [
            alphas_cumprod[timesteps_table[i + 1].long()]
            if i < last
    ...
    std_dev_t = (
        eta
        * (beta_prev / beta_t).sqrt()
        * (1.0 - abar_t / abar_prev).sqrt()
    )

    prev_sample_mean = (
        abar_prev.sqrt() * x0_hat
        + (beta_prev - std_dev_t**2).clamp_min(0.0).sqrt() * eps_hat
    )
