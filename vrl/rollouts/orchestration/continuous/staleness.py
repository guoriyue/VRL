...
class StalenessPolicy:
    ...
    def __post_init__(self) -> None:
        require_exact_int(
            self.max_stale_policy_versions, path="max_stale_policy_versions", minimum=0
        )
    ...
    def staleness(
        ...
        if item_version is not None:
            require_exact_int(item_version, path="item_policy_version", minimum=0)
        if current_version is not None:
            require_exact_int(current_version, path="current_policy_version", minimum=0)
