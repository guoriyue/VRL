...
def save_training_checkpoint(
    ...
    *,
    ...
    try:
        # ``checkpoint_state_dict`` is the FSDP-aware trainer snapshot: it joins
        # the optimizer/EMA DTensor gathers on every rank but retains the full
        # tree only on the primary, so FP32-master runs checkpoint correctly.
        # Trainers without it fall back to the plain state_dict.
