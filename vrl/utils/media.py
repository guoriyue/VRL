...
def video_tensor_to_uint8_frames(tensor: torch.Tensor) -> np.ndarray:
    ...
    # .cpu(): this converter ends in .numpy(), which requires a host tensor. Both
    # rollout paths copy batch results to the CPU before they leave the worker,
    # but in-process callers can still pass CUDA tensors. Mirror
    # image_to_uint8_hwc's intake so any device works.
