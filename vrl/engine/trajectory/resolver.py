...
def require_batch_trajectory(batch: Any) -> TrajectoryBatch:
    """Return the first-class trajectory attached to a trainer batch."""
    ...
    validate_trajectory_batch(trajectory)
    return trajectory


def primary_trainable_segment_name(batch: Any, fallback: str | None = None) -> str:
    """Resolve the primary trainable segment for replay from a trainer batch."""

    trajectory = require_batch_trajectory(batch)
    training_view = getattr(batch, "training_view", None)
    if isinstance(training_view, TrainingView) and training_view.primary_segment:
...
def trajectory_tensor_value(
    batch: Any,
    segment_name: str,
    tensor_name: str,
) -> Any:
    """Read one named tensor from a batch trajectory."""

    trajectory = require_batch_trajectory(batch)
    segment = trajectory.segments.get(segment_name)
    if segment is None:
...
def trajectory_role_value(
    batch: Any,
    segment_name: str,
    role: TensorRole,
) -> Any:
    """Read the unique tensor with ``role`` from a batch trajectory segment."""
...
def trajectory_replay_tensor_dict(
    batch: Any,
    ...
    *,
    ...
    trajectory = require_batch_trajectory(batch)
    name = segment_name or primary_trainable_segment_name(batch)
    segment = trajectory.segments.get(name)
    if segment is None:
    ...
    for ref in replay.tensor_refs:
        segment_ref, tensor_name = _split_ref(_canonical_tensor_ref(name, ref), "tensor")
        if segment_ref != name:
            _fail(
                f"replay input {name}.{replay_input_name} crosses segment boundary "
                f"with tensor ref {ref!r}",
            )
        out[tensor_name] = trajectory_tensor_value(batch, segment_ref, tensor_name)
    return out
...
def resolve_training_view(
    ...
    """Resolve a training view into typed tensors owned by ``trajectory``."""

    validate_trajectory_batch(trajectory)
    validate_training_view(trajectory, view)
    resolved_units = tuple(_resolve_loss_unit(trajectory, unit) for unit in view.loss_units)
...
def resolve_loss_unit(
    trajectory: TrajectoryBatch,
    unit: LossUnit,
) -> ResolvedLossUnit:
    """Resolve a single loss unit after validating it against ``trajectory``."""
    ...
    return _resolve_loss_unit(trajectory, unit)


def _resolve_loss_unit(
    trajectory: TrajectoryBatch,
    unit: LossUnit,
) -> ResolvedLossUnit:
    segment = trajectory.segments.get(unit.segment)
    if segment is None:
        _fail(f"LossUnit.segment={unit.segment!r} is unknown")

    action = _resolve_tensor(
        trajectory,
        unit,
        unit.action_ref,
    ...
    replay_inputs = tuple(
        _resolve_replay_input(trajectory, unit, replay_ref)
...
def _resolve_replay_input(
    ...
    resolved_tensors = tuple(
        _resolve_tensor(
            trajectory,
            unit,
            _canonical_tensor_ref(segment_name, tensor_ref_value),
            expected_role=None,
    ...
    context_values = tuple(
        _resolve_context_value(trajectory, context_ref)
...
def _resolve_tensor(
    ...
    *,
    ...
    source = _lookup_tensor(trajectory, ref)
    ...
    _require_axis_shape(trajectory, ref, value, axes)
...
def _lookup_tensor(trajectory: TrajectoryBatch, ref: str) -> TrajectoryTensor:
...
def _resolve_context_value(
    ...
    if ref not in trajectory.context:
        _fail(f"ReplayInput references unknown context value {ref!r}")
    return ResolvedContextValue(ref=ref, value=trajectory.context[ref])


def _canonical_tensor_ref(segment_name: str, ref: str) -> str:
    if "." in ref:
        return ref
