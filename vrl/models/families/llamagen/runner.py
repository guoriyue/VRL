...
class LlamaGenARModelRunner(ARDiscreteTokenRunner):
    ...
    def init_token(
        ...
        *,
        ...
        masks = torch.cat([cond_attention_mask, uncond_attention_mask], dim=0).to(device)
        trunk.causal_mask[:, :, :caption_len] = trunk.causal_mask[
            :, :, :caption_len
        ] * masks.unsqueeze(1)
        eye = torch.eye(trunk.causal_mask.size(1), trunk.causal_mask.size(2), device=device)
    ...
    def _validate_family_step(
        ...
        if batch.row_indices != list(range(batch_size)):
            raise ValueError(
                ...
                "scheduler_batch_size unset or >= batch sample count."
            )
