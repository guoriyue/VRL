"""Wan 2.1 official t2v adapter (DiffusionPolicy contract).

Single-protocol adapter for the official Wan T2V codebase. The contract is:

    encode_prompt -> prepare_sampling -> forward_step xN -> decode_latents

Scope is intentionally narrowed to T2V on the RL path; image-to-video is
not supported. ``forward_step`` raises ``NotImplementedError`` for
non-T2V requests.

The meaningful difference vs the diffusers adapter is the **boundary
expert routing**: the official Wan pipeline carries two transformer
experts (``high_noise_model`` / ``low_noise_model``) and we pick which
one to forward through based on whether the current timestep is above
or below the configured boundary timestep.
"""

from __future__ import annotations

import hashlib
import importlib
import math
import random
import sys
from dataclasses import dataclass, field
from pathlib import Path
from typing import Any

from vrl.models.diffusion import DiffusionPolicy, VideoGenerationRequest


def _stable_hash(value: str) -> str:
    """Short stable hash for Wan official prompt-cache keys."""

    return hashlib.sha256(value.encode("utf-8")).hexdigest()[:16]


def _clone_tensor_list_to_cpu(tensors: list[Any]) -> list[Any]:
    return [tensor.detach().cpu() for tensor in tensors]


def _move_tensor_list_to_device(tensors: list[Any], device: Any) -> list[Any]:
    return [tensor.to(device) for tensor in tensors]


@dataclass
class WanT2VOfficialSamplingState:
    """Private Wan official T2V sampling state. Engine MUST NOT introspect."""

    latents: list[Any]  # official wan stores latents as list[Tensor]
    timesteps: Any
    scheduler: Any
    seed_generator: Any
    arg_c: dict = field(default_factory=dict)
    arg_null: dict = field(default_factory=dict)
    boundary: float = 0.0
    high_noise_guidance_scale: float = 5.0
    low_noise_guidance_scale: float = 5.0
    pipeline: Any = None
    task_key: str = ""
    seed: int = 0
    solver_name: str = "dpmpp"


class WanT2VOfficialPolicy(DiffusionPolicy):
    """Official Wan2.2 in-process T2V adapter (RL training path)."""

    family = "wan-official-t2v"

    def __init__(
        self,
        *,
        repo_dir: str | Path,
        default_checkpoint_dir: str | Path | None = None,
        device_id: int = 0,
        default_t5_cpu: bool = True,
        default_convert_model_dtype: bool = True,
    ) -> None:
        super().__init__()
        self.repo_dir = Path(repo_dir)
        self.default_checkpoint_dir = (
            None if default_checkpoint_dir is None else Path(default_checkpoint_dir)
        )
        self.device_id = device_id
        self.default_t5_cpu = default_t5_cpu
        self.default_convert_model_dtype = default_convert_model_dtype

        self._modules_loaded = False
        self._torch = None
        self._wan = None
        self._wan_configs = None
        self._size_configs = None
        self._max_area_configs = None
        self._pipelines: dict[str, Any] = {}
        self._prompt_cache: dict[str, tuple[list[Any], list[Any]]] = {}

    async def load(self) -> None:
        self._load_modules()

    # -- backend ownership (called by runtime, not by collectors) -------

    @classmethod
    def from_spec(cls, spec: Any) -> WanT2VOfficialPolicy:
        """Construct from a RuntimeBuildSpec.

        ``spec.model_name_or_path`` -> ``repo_dir`` (path to wan repo clone).
        Optional ``spec.native_backend_config`` carries device_id / t5_cpu /
        convert_model_dtype / default_checkpoint_dir overrides.
        """
        cfg = spec.native_backend_config or {}
        return cls(
            repo_dir=spec.model_name_or_path,
            default_checkpoint_dir=cfg.get("default_checkpoint_dir"),
            device_id=int(cfg.get("device_id", 0)),
            default_t5_cpu=bool(cfg.get("t5_cpu", True)),
            default_convert_model_dtype=bool(cfg.get("convert_model_dtype", True)),
        )

    def apply_lora(self, spec: Any) -> None:
        raise NotImplementedError(
            "Official Wan adapter does not support LoRA on the RL path yet "
            "(dual high_noise / low_noise expert transformers + per-request "
            "lazy load). Use the diffusers backend if you need LoRA training.",
        )

    def enable_full_finetune(self) -> None:
        """No-op: official path lazy-loads experts per request; trainable
        flags are handled implicitly by the wan library."""

    def torch_compile_transformer(self, mode: str) -> None:
        raise NotImplementedError(
            "Official Wan adapter does not support torch.compile on the RL "
            "path yet (dual experts + per-request load).",
        )

    def set_num_steps(self, n: int) -> None:
        """No-op: official Wan constructs its scheduler per request inside
        ``prepare_sampling`` from ``request.num_steps``; there's no global
        scheduler to mutate at build time."""

    @property
    def trainable_modules(self) -> dict[str, Any]:
        """Loaded experts, if any. Empty until first request triggers load."""
        out: dict[str, Any] = {}
        for cache_key, pipeline in self._pipelines.items():
            for attr in ("low_noise_model", "high_noise_model"):
                model = getattr(pipeline, attr, None)
                if model is not None:
                    out[f"{cache_key}/{attr}"] = model
        return out

    @property
    def scheduler(self) -> Any:
        """No global scheduler — built per request. Returns None."""
        return None

    @property
    def backend_handle(self) -> Any:
        """The per-request pipeline cache (raw wan library objects)."""
        return self._pipelines

    def describe(self) -> dict[str, Any]:
        return {
            "family": self.family,
            "repo_dir": str(self.repo_dir),
            "default_checkpoint_dir": (
                None if self.default_checkpoint_dir is None
                else str(self.default_checkpoint_dir)
            ),
            "device": f"cuda:{self.device_id}",
        }

    # -- module loading ------------------------------------------------

    def _load_modules(self) -> None:
        if self._modules_loaded:
            return
        if not self.repo_dir.exists():
            raise FileNotFoundError(f"Wan repo_dir does not exist: {self.repo_dir}")
        repo_str = str(self.repo_dir)
        if repo_str not in sys.path:
            sys.path.insert(0, repo_str)
        import torch
        import wan
        import wan.modules.attention as wan_attention
        import wan.modules.model as wan_model
        from wan.configs import MAX_AREA_CONFIGS, SIZE_CONFIGS, WAN_CONFIGS

        if not wan_attention.FLASH_ATTN_2_AVAILABLE and not wan_attention.FLASH_ATTN_3_AVAILABLE:
            wan_model.flash_attention = wan_attention.attention

        self._torch = torch
        self._wan = wan
        self._wan_configs = WAN_CONFIGS
        self._size_configs = SIZE_CONFIGS
        self._max_area_configs = MAX_AREA_CONFIGS
        self._modules_loaded = True

    # -- pipeline lookup -----------------------------------------------

    def _task_key(self, task_type: str, model_size: str) -> str:
        if task_type == "text_to_video":
            return f"t2v-{model_size}"
        raise NotImplementedError(
            "WanT2VOfficialPolicy does not support i2v on the RL path; "
            "i2v not supported on RL path",
        )

    def _size_key(self, width: int, height: int) -> str:
        size_key = f"{width}*{height}"
        if size_key not in self._size_configs:
            raise ValueError(
                f"Wan official model does not support size {size_key}; "
                f"supported sizes are {sorted(self._size_configs.keys())}"
            )
        return size_key

    def _resolve_checkpoint_dir(self, ckpt_dir: str | None, task_key: str) -> Path:
        del task_key
        if ckpt_dir:
            return Path(ckpt_dir)
        if self.default_checkpoint_dir is not None:
            return self.default_checkpoint_dir
        raise FileNotFoundError(
            "No Wan checkpoint directory configured. Set request.ckpt_dir or "
            "WanT2VOfficialPolicy.default_checkpoint_dir."
        )

    def _validate_checkpoint_layout(
        self, task_key: str, checkpoint_dir: Path, config: Any,
    ) -> None:
        required_paths: list[tuple[Path, str]] = [
            (checkpoint_dir / config.t5_checkpoint, "T5 checkpoint"),
            (checkpoint_dir / config.t5_tokenizer, "T5 tokenizer"),
            (checkpoint_dir / config.vae_checkpoint, "VAE checkpoint"),
        ]
        for path, label in required_paths:
            if not path.exists():
                raise FileNotFoundError(
                    f"Incomplete Wan checkpoint layout for {task_key}: "
                    f"missing {label} at {path}"
                )
        for subfolder_attr, label in (
            ("low_noise_checkpoint", "low-noise"),
            ("high_noise_checkpoint", "high-noise"),
        ):
            subfolder = getattr(config, subfolder_attr, None)
            if subfolder is None:
                continue
            shard_dir = checkpoint_dir / subfolder
            has_weight = any(shard_dir.glob("*.safetensors")) or any(shard_dir.glob("*.bin"))
            has_index = any(shard_dir.glob("*.index.json"))
            if not has_weight and not has_index:
                raise FileNotFoundError(
                    f"Incomplete Wan checkpoint layout for {task_key}: "
                    f"{label} model weights are missing under {shard_dir}"
                )

    def _pipeline_cache_key(
        self,
        task_key: str,
        checkpoint_dir: Path,
        t5_cpu: bool,
        convert_model_dtype: bool,
    ) -> str:
        return "|".join(
            [
                task_key,
                str(checkpoint_dir),
                str(self.device_id),
                str(t5_cpu),
                str(convert_model_dtype),
            ]
        )

    def _model_for_timestep(self, pipeline: Any, timestep: Any, boundary: float) -> Any:
        """Select active Wan expert by timestep boundary (offload variant)."""
        active_is_high = timestep.item() >= boundary
        active = pipeline.high_noise_model if active_is_high else pipeline.low_noise_model
        inactive = pipeline.low_noise_model if active_is_high else pipeline.high_noise_model
        if next(inactive.parameters()).device.type == "cuda":
            inactive.to("cpu")
        if next(active.parameters()).device.type == "cpu":
            active.to(pipeline.device)
        return active

    def _get_pipeline(self, request: VideoGenerationRequest) -> tuple[Any, str, Path]:
        self._load_modules()
        task_key = self._task_key(request.task_type, request.model_size)
        checkpoint_dir = self._resolve_checkpoint_dir(request.ckpt_dir, task_key)
        t5_cpu = request.t5_cpu if request.t5_cpu is not None else self.default_t5_cpu
        convert_dtype = (
            request.convert_model_dtype
            if request.convert_model_dtype is not None
            else self.default_convert_model_dtype
        )
        cache_key = self._pipeline_cache_key(task_key, checkpoint_dir, t5_cpu, convert_dtype)
        pipeline = self._pipelines.get(cache_key)
        if pipeline is not None:
            return pipeline, task_key, checkpoint_dir

        config = self._wan_configs[task_key]
        self._validate_checkpoint_layout(task_key, checkpoint_dir, config)
        common_kwargs = {
            "config": config,
            "checkpoint_dir": str(checkpoint_dir),
            "device_id": self.device_id,
            "rank": 0,
            "t5_fsdp": False,
            "dit_fsdp": False,
            "use_sp": False,
            "t5_cpu": t5_cpu,
            "convert_model_dtype": convert_dtype,
        }
        if not task_key.startswith("t2v-"):
            raise NotImplementedError(
                "WanT2VOfficialPolicy does not support i2v on the RL path; "
                "i2v not supported on RL path",
            )
        pipeline = self._wan.WanT2V(**common_kwargs)
        self._pipelines[cache_key] = pipeline
        return pipeline, task_key, checkpoint_dir

    # -- encode_prompt -------------------------------------------------

    def encode_prompt(
        self,
        prompt: str | list[str],
        negative_prompt: str | list[str] | None = None,
        **kwargs: Any,
    ) -> dict[str, Any]:
        """Encode prompt via the official Wan T5 text encoder.

        Returns the cond/uncond context tensor lists plus opaque pipeline /
        task-key handles that ``prepare_sampling`` consumes.
        """
        request: VideoGenerationRequest | None = kwargs.get("request")
        if request is None:
            raise ValueError(
                "WanT2VOfficialPolicy.encode_prompt requires request=... "
                "in kwargs to resolve checkpoint / task-key."
            )
        pipeline, task_key, checkpoint_dir = self._get_pipeline(request)
        torch = self._torch
        prompt_str = prompt if isinstance(prompt, str) else prompt[0]
        n_prompt = (
            negative_prompt
            if isinstance(negative_prompt, str) and negative_prompt
            else (negative_prompt[0] if isinstance(negative_prompt, list) and negative_prompt
                  else pipeline.sample_neg_prompt)
        )
        prompt_hash = _stable_hash(f"{request.model_name}|{prompt_str}|{n_prompt}")
        cache_key = f"{task_key}|{checkpoint_dir}|{prompt_hash}"

        if cache_key in self._prompt_cache:
            cached_context, cached_context_null = self._prompt_cache[cache_key]
        else:
            if not pipeline.t5_cpu:
                pipeline.text_encoder.model.to(pipeline.device)
                ctx = pipeline.text_encoder([prompt_str], pipeline.device)
                ctx_null = pipeline.text_encoder([n_prompt], pipeline.device)
                if request.offload_model:
                    pipeline.text_encoder.model.cpu()
            else:
                ctx = pipeline.text_encoder([prompt_str], torch.device("cpu"))
                ctx_null = pipeline.text_encoder([n_prompt], torch.device("cpu"))
            cached_context = _clone_tensor_list_to_cpu(ctx)
            cached_context_null = _clone_tensor_list_to_cpu(ctx_null)
            self._prompt_cache[cache_key] = (cached_context, cached_context_null)

        device_context = _move_tensor_list_to_device(cached_context, pipeline.device)
        device_context_null = _move_tensor_list_to_device(cached_context_null, pipeline.device)
        return {
            "text_context": device_context,
            "text_context_null": device_context_null,
            "_pipeline": pipeline,
            "_task_key": task_key,
            "_checkpoint_dir": str(checkpoint_dir),
        }

    # -- prepare_sampling ----------------------------------------------

    def prepare_sampling(
        self,
        request: VideoGenerationRequest,
        encoded: dict[str, Any],
        **kwargs: Any,
    ) -> WanT2VOfficialSamplingState:
        """Build per-request sampling state for the official Wan T2V loop.

        Builds target shape, seed-pinned random latents, ``arg_c`` /
        ``arg_null`` plumbing, and instantiates the wan-library scheduler.
        """
        if request.task_type != "text_to_video":
            raise NotImplementedError(
                "WanT2VOfficialPolicy does not support i2v on the RL path; "
                "i2v not supported on RL path",
            )

        pipeline = encoded["_pipeline"]
        task_key = encoded["_task_key"]
        torch = self._torch

        size_key = self._size_key(request.width, request.height)
        seed = (
            request.seed if request.seed is not None else random.randint(0, sys.maxsize)
        )
        seed_g = torch.Generator(device=pipeline.device)
        seed_g.manual_seed(seed)

        low_noise_guidance_scale = float(request.guidance_scale)
        high_noise_guidance_scale = (
            low_noise_guidance_scale
            if request.high_noise_guidance_scale is None
            else float(request.high_noise_guidance_scale)
        )

        width, height = self._size_configs[size_key]
        target_shape = (
            pipeline.vae.model.z_dim,
            (request.frame_count - 1) // pipeline.vae_stride[0] + 1,
            height // pipeline.vae_stride[1],
            width // pipeline.vae_stride[2],
        )
        seq_len = (
            math.ceil(
                (target_shape[2] * target_shape[3])
                / (pipeline.patch_size[1] * pipeline.patch_size[2])
                * target_shape[1]
                / pipeline.sp_size
            )
            * pipeline.sp_size
        )
        latents = [
            torch.randn(
                target_shape[0],
                target_shape[1],
                target_shape[2],
                target_shape[3],
                dtype=torch.float32,
                device=pipeline.device,
                generator=seed_g,
            )
        ]
        arg_c = {"context": encoded["text_context"], "seq_len": seq_len}
        arg_null = {"context": encoded["text_context_null"], "seq_len": seq_len}

        boundary = pipeline.boundary * pipeline.num_train_timesteps
        solver_name = request.sample_solver
        if solver_name == "unipc":
            scheduler = importlib.import_module(
                "wan.utils.fm_solvers_unipc"
            ).FlowUniPCMultistepScheduler(
                num_train_timesteps=pipeline.num_train_timesteps,
                shift=1,
                use_dynamic_shifting=False,
            )
            scheduler.set_timesteps(
                request.num_steps, device=pipeline.device, shift=request.shift,
            )
            timesteps = scheduler.timesteps
        else:
            fm_solvers = importlib.import_module("wan.utils.fm_solvers")
            scheduler = fm_solvers.FlowDPMSolverMultistepScheduler(
                num_train_timesteps=pipeline.num_train_timesteps,
                shift=1,
                use_dynamic_shifting=False,
            )
            sampling_sigmas = fm_solvers.get_sampling_sigmas(
                request.num_steps, request.shift,
            )
            timesteps, _ = fm_solvers.retrieve_timesteps(
                scheduler,
                device=pipeline.device,
                sigmas=sampling_sigmas,
            )

        return WanT2VOfficialSamplingState(
            latents=latents,
            timesteps=timesteps,
            scheduler=scheduler,
            seed_generator=seed_g,
            arg_c=arg_c,
            arg_null=arg_null,
            boundary=boundary,
            high_noise_guidance_scale=high_noise_guidance_scale,
            low_noise_guidance_scale=low_noise_guidance_scale,
            pipeline=pipeline,
            task_key=task_key,
            seed=seed,
            solver_name=solver_name,
        )

    # -- forward_step --------------------------------------------------

    def forward(
        self,
        state: WanT2VOfficialSamplingState,
        step_idx: int,
    ) -> dict[str, Any]:
        """Run one official Wan step without assuming a single transformer."""

        return self.forward_step(state, step_idx)

    def forward_step(
        self,
        state: WanT2VOfficialSamplingState,
        step_idx: int,
    ) -> dict[str, Any]:
        """One Wan transformer forward + CFG. NO scheduler step.

        The boundary rule picks ``high_noise_model`` if ``t >= boundary`` else
        ``low_noise_model``.
        """
        torch = self._torch
        pipeline = state.pipeline

        if state.task_key and not state.task_key.startswith("t2v-"):
            raise NotImplementedError(
                "WanT2VOfficialPolicy.forward_step is t2v-only",
            )

        t = state.timesteps[step_idx]
        timestep = torch.stack([t]).to(pipeline.device)

        m = (
            pipeline.high_noise_model
            if t.item() >= state.boundary
            else pipeline.low_noise_model
        )

        sample_guide_scale = (
            state.high_noise_guidance_scale
            if t.item() >= state.boundary
            else state.low_noise_guidance_scale
        )

        noise_pred_cond = m(state.latents, t=timestep, **state.arg_c)[0]
        noise_pred_uncond = m(state.latents, t=timestep, **state.arg_null)[0]
        noise_pred = noise_pred_uncond + sample_guide_scale * (
            noise_pred_cond - noise_pred_uncond
        )

        return {
            "noise_pred": noise_pred,
            "noise_pred_cond": noise_pred_cond,
            "noise_pred_uncond": noise_pred_uncond,
        }

    # -- collector boundary --------------------------------------------

    def export_batch_context(self, state: WanT2VOfficialSamplingState) -> dict[str, Any]:
        """Project SamplingState -> RolloutBatch.context (shared metadata).

        Note: official adapter is not yet wired into the RL runtime; this
        keeps the contract uniform without committing to behavior the path
        does not exercise yet.
        """
        return {
            "guidance_scale": state.low_noise_guidance_scale,
            "high_noise_guidance_scale": state.high_noise_guidance_scale,
            "boundary": state.boundary,
            "cfg": state.low_noise_guidance_scale > 1.0,
            "model_family": self.family,
            "task_key": state.task_key,
        }

    def export_training_extras(self, state: WanT2VOfficialSamplingState) -> dict[str, Any]:
        """Official adapter does not pack per-sample embeds for eval today."""
        return {}

    def restore_eval_state(self, *args: Any, **kwargs: Any) -> Any:
        raise NotImplementedError(
            "Official Wan adapter does not support eval-path forward yet "
            "(LoRA training on dual experts is gated; see apply_lora).",
        )

    # -- decode_latents ------------------------------------------------

    def decode_latents(self, latents: Any) -> Any:
        """Decode latents -> video via the official Wan VAE.

        Accepts either a single tensor or the official ``list[Tensor]``
        layout used by the sampling state.
        """
        if not self._modules_loaded:
            raise RuntimeError(
                "WanT2VOfficialPolicy.decode_latents called before modules "
                "are loaded; call ``load()`` or run ``encode_prompt`` first.",
            )
        # The official VAE expects list[Tensor]; pick the first cached
        # pipeline if a sampling state didn't carry one through.
        if not self._pipelines:
            raise RuntimeError(
                "WanT2VOfficialPolicy.decode_latents requires a loaded pipeline; "
                "no pipeline cached. Call ``encode_prompt`` first.",
            )
        # Use the only / most recently used pipeline
        pipeline = next(iter(self._pipelines.values()))
        latents_list = latents if isinstance(latents, list) else [latents]
        video = pipeline.vae.decode(latents_list)[0]
        return video
