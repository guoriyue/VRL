...
def glm_image_grid_dims(height: int, width: int) -> tuple[int, int, int, int]:
    ...
    if height < factor or width < factor or height % factor or width % factor:
        raise ValueError(
            f"GLM-Image image size must be positive multiples of {factor}, "
...
def glm_image_decode_position_schedule(
    ...
    tests/models/ar/glm_image/test_token_schedule.py:
...
def glm_image_prefill_position_ids(attention_mask: torch.Tensor) -> torch.Tensor:
    ...
    position_ids = torch.ones(
        3, batch, length, dtype=torch.long, device=attention_mask.device,
    )
    for row in range(batch):
        valid = attention_mask[row] == 1
        count = int(valid.sum())
        text_positions = torch.arange(
            count, dtype=torch.long, device=attention_mask.device,
        )
...
class GlmImageModel(ARModelBase):
    ...
    def _require_surface(self, attrs: tuple[str, ...]) -> None:
        for attr in attrs:
            if not hasattr(self.glm, attr):
                raise RuntimeError(
                    f"Loaded model is missing `{attr}` — does not look like "
                    "a GlmImageForConditionalGeneration checkpoint."
                )

    def _require_inner_surface(self, attrs: tuple[str, ...]) -> None:
        for attr in attrs:
            if not hasattr(self.glm.model, attr):
                raise RuntimeError(
                    f"Loaded model is missing `model.{attr}` — does not look "
                    "like a GlmImageForConditionalGeneration checkpoint."
                )
    ...
    def _apply_lora(self) -> None:
        ...
        self.glm.model.language_model = get_peft_model(
            self.glm.model.language_model, lora_cfg,
        )
        logger.info(
    ...
    def replay_forward(
        ...
        *,
        ...
        logits = self.forward_image_logits(
            prompt_embeds, prompt_mask, image_token_ids, grids=grids,
        )  # [B, L_img, V_codebook]
        return ReplayResult(
            segments={
    ...
    def _replay_grids(
        ...
        if height is None or width is None:
            raise RuntimeError(
                "GLM-Image replay requires image_height/image_width in the "
                "rollout context to rebuild the mrope position schedule.",
            )
        height, width = int(height), int(width)
        expected = glm_image_token_num(height, width)
        if expected != token_count:
            raise RuntimeError(
                f"GLM-Image replay token count {token_count} does not match "
                f"the {height}x{width} target (expected {expected}).",
            )
        token_h, token_w, prev_h, prev_w = glm_image_grid_dims(height, width)
        return ((token_h, token_w), (prev_h, prev_w))
    ...
    def decode_image_tokens(
        ...
        *,
        ...
        prior_token_ids = torch.cat(
            [
                _upsample_token_ids(large[row], token_h, token_w)
                for row in range(B)
            ],
            dim=0,
        )
