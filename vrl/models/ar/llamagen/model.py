...
Mirrors ``vrl.models.ar.janus_pro.JanusProModel`` for FoundationVision's
...
def _load_llamagen_gpt(config: LlamaGenConfig) -> nn.Module:
    from vrl.models.ar.llamagen.vendor.gpt import GPT_models

    if config.gpt_model not in GPT_models:
        raise ValueError(
            f"unknown LlamaGen gpt_model {config.gpt_model!r}; "
            f"known: {sorted(GPT_models)}"
        )
...
def _load_llamagen_vq(config: LlamaGenConfig) -> nn.Module:
    from vrl.models.ar.llamagen.vendor.vq_model import VQ_models
