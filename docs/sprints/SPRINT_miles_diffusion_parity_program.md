...
## 5. strict arm vs recompute arm：40 epoch 完整对照（2026-09-15 17:00）

两条 arm 同配方（SD3.5 OCR dedicated 3x1，trainer GPU 0、rollout GPU 1-3、托管 CPU OCR，seed 1234，
40 epoch）；差异只有 recompute arm 的三个开关：`samples_per_generation_batch=16`（strict 为 1）、
`model.torch_compile.scope=all`（strict 关）、`precision_correction.recompute_old_logprob=on`（strict 关）。
strict arm 的 parity 为 0、clip 为 0（同形状生成/重放）；recompute arm 的 parity 0.018–0.021 被记录
但不进梯度，clip 亦为 0。

| epoch 段 | strict reward | recompute reward | strict std | recompute std | strict adv_zero | recompute adv_zero | recompute parity | recompute clip | strict wall s | recompute wall s |
|---|---|---|---|---|---|---|---|---|---|---|
| 0–4 | 0.439 | 0.406 | 0.310 | 0.318 | 0.012 | 0.005 | 0.0196 | 0.000 | 510 | 308 |
| 5–9 | 0.497 | 0.399 | 0.313 | 0.303 | 0.009 | 0.006 | 0.0194 | 0.000 | 516 | 300 |
| 10–14 | 0.430 | 0.365 | 0.295 | 0.286 | 0.008 | 0.028 | 0.0160 | 0.000 | 510 | 289 |
| 15–19 | 0.419 | 0.343 | 0.318 | 0.287 | 0.002 | 0.000 | 0.0196 | 0.000 | 517 | 291 |
| 20–24 | 0.420 | 0.276 | 0.301 | 0.259 | 0.002 | 0.003 | 0.0262 | 0.000 | 519 | 287 |
| 25–29 | 0.496 | 0.344 | 0.316 | 0.281 | 0.000 | 0.000 | 0.0207 | 0.000 | 515 | 290 |
| 30–34 | 0.468 | 0.315 | 0.310 | 0.281 | 0.005 | 0.033 | 0.0202 | 0.000 | 516 | 278 |
| 35–39 | 0.415 | 0.332 | 0.291 | 0.254 | 0.000 | 0.006 | 0.0165 | 0.000 | 517 | 289 |

strict 全程均值 0.448（epoch 间 std 0.075），前半 0.446 → 后半 0.450；recompute 全程均值 0.347（std 0.077），前半 0.378 → 后半 0.317。epoch 墙钟 strict 515 s vs recompute 291 s（1.77×）。

<details><summary>逐 epoch reward</summary>

| epoch | strict | recompute |
|---|---|---|
| 0 | 0.412 | 0.425 |
| 1 | 0.476 | 0.392 |
| 2 | 0.273 | 0.264 |
...
**读法**：epoch 间 std 约 0.075，5-epoch 段均值的标准误约 0.034。strict 前后半 0.446 → 0.450（平坦）；
recompute 0.378 → 0.317（下降 0.06，约 2 个标准误），且 8 个 5-epoch 段里 7 个低于 strict。速度结论
（1.77× 全程均值；含 compile 预热的首 epoch）与 clip 结论成立，学习结论**不成立**：该组合在这套配方下
让策略退化。三个变量的归因交给消融（A 去 compile、B 去 batch-16、C 去 recompute，各 20 epoch，
`run_recompute_ablations.sh`），GPU 在 vrl-9941 的任务后可用（约 18:40）。

**当前判定对默认配置的含义**：`recompute_old_logprob=on`、batch-16 生成、compile(all) 三者都**不进**
SD3.5 默认预设；strict 配方（batch 1、eager、bypass）仍是默认。PAUSE 已于 13:36 撤除，vrl-9941 已通知。
