...
> **Current-state correction (2026-07-13).** This parked document preserves old
> measurements and design alternatives; it does not define the current API. The
> canonical diffusion implementation is
> `DiffusionChunkExecutorBase.forward_chunk_plan`, which directly runs the typed
> prompt-encode, prepare, denoise, and decode methods. The optional single-worker
> request pipeline calls that same canonical chunk forward while the previous
> chunk's D2H teardown runs on a side stream. The former logical topology,
> payload/result, serial runner, and Ray stage adapters were deleted after they
> were proven test-only. Current multi-worker rollout is chunk-level data
> parallelism across full-model workers, not physical stage placement. Reward
> `gpu_pool` remains role-level ownership; reward/generation streaming separately
> requires nonblocking transport and verified accelerator isolation. Any later
> reference to the deleted seam records historical code or proposal vocabulary.
...
> - **小 idle → 未自动处理:**
>   - **② Per-worker pipelines remain unimplemented.** `pipelined=true` now rejects more than one rollout worker instead of silently switching to per-chunk dispatch. Supporting multiple workers requires an explicit request partition for each worker and multi-GPU validation; it is not implied by this single-worker optimization.
...
>
> ---
> # Historical direction recorded on 2026-06-27
>
> The measurements in this block remain evidence. Its topology/runner scaffolding
> statements are superseded by the 2026-07-13 correction above.
>
...
> ② 跨卡:  reward 切专卡（已建 async-reward enabler: reward.gpu_pool=dedicated)→ 藏 14%   ← role-level resource placement; not physical `PipelineStage` placement
...
>      - `RayGenerationExecutor`: `pipelined` remains opt-in and single-worker only. Config resolution and direct executor construction both reject multiple workers. Requests need at least two planned chunks; one-chunk requests keep the normal per-chunk path because there is nothing to overlap.
>      - config 接好:`distributed.rollout.pipelined`(config.py 解析 + launcher.py 传参)。
>      - **Pipeline OOM is now recoverable without a speculative memory estimator.** The worker clears partial pipeline state and returns a typed OOM result; the driver retries through the existing per-chunk CPU handoff and recursive split-on-OOM path. Non-OOM failures still propagate. The fallback and one-chunk admission are covered in `tests/generation/ray/test_oom_split.py` and `tests/generation/execution/test_execute_request_pipelined.py`.
...
>   `run_chunk_through_pipeline` as a serial equivalence seam. The 2026-07-13
>   audit found that it duplicated `forward_chunk_plan` exactly and deleted it.
...
> - The historical serial and Ray runners routed one payload sequentially. They
>   had no production constructor and were deleted; request pipelining does not
>   use them.
...
> - **Current conclusion:** single-worker request pipelining has a production
>   entry. No physical-stage contract is retained; multi-worker stage pipelining
>   remains gated by profiling.

Historical status: commit `b224383` introduced typed executor-stage outputs and
an unwired topology/runner test seam. The typed model-stage methods remain in the
canonical `forward_chunk_plan`; the generic topology/runner seam and its tests
were later deleted. Physical T2-T6 work remains closed until profiling proves it
beats data-parallel rollout.
...
This section is a historical design interpretation. Its measurements remain
useful, but its "stage placement" wording predates the current distinction:
reward GPU selection is role-level resource placement, while physical pipeline
stage placement remains unimplemented.
...
Placing the reward runtime on a 2nd GPU can run reward(group N) concurrently with
rollout(group N+1) AND removes the offload/restore barrier — hiding the full 14% +
the barrier. This is role-level reward placement, not §1's proposed physical
pipeline-stage placement. Safe overlap additionally requires a nonblocking reward
runtime and verified accelerator isolation.
...
```text
on 1 GPU now  : UNIT-test async-reward control flow and role-level placement
...
So: the useful scope is **async reward + role-level reward placement**,
correctness-validated on one GPU; the throughput payoff is gated to a second GPU.
Do NOT infer a physical stage API from this result or build generic
encode/VAE/relay machinery first.

### 4.4 2026-06-27 历史复核 — async reward / role-level placement

§4.2 / §4.3 把 reward 定为 gate-open 的 prize。当时文档把 role-level
reward placement 叫成 per-stage placement，这一术语已被顶部 correction 取代。本节是**对照当时测试与配置**的历史复核：当时的 do-next
...
所以 reward role 的 GPU-pool 配置面已经有了，连 auto-pick-spare-GPU 都已实现。
这不是 §1 设想的 physical `PipelineStage` placement 配置面。

**(c) continuous producer 有多 group inflight 调度能力；reward/generation overlap 另行 gated。** producer 用 bounded
...
vrl/rollouts/orchestration/continuous/producer.py
  ...
  _run 循环按 max_inflight_groups 维持多组在飞
  reward(N) / generate(N+1) 是否真实重叠还受 runtime capability gate 限制
...
即「bounded-async schedule」的多 group inflight 调度器已经存在；是否实际执行
reward(group N) ∥ rollout(group N+1) 由 reward runtime capability 决定，不能只从
`max_inflight_groups` 推断。
...
**Historical validation recorded by that revision:**
...
vrl/models/diffusion/common/latent_decode.py
vrl/models/diffusion/common/vae_decode_memory.py
