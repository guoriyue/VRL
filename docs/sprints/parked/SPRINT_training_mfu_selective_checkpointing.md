...
状态：**parked / P2 已落地 / 剩 P3 GPU-gated（2026-07-09 对账）**。恢复触发：有可用于 P3 对照的 GPU 时。性质：**把现在 full / 二值的 gradient checkpointing 换成 selective（只重算便宜的 norm/elementwise，保留贵的 GEMM/attention 激活），把腾出的显存花在更大的 microbatch 上 → 抬算术强度 → 抬训练 MFU。** 这是 [[SPRINT_training_mfu_compile]] 的姊妹杠杆：compile 靠融合腾显存，本 sprint 靠「只重算便宜的」摘掉 recompute 税，两者都服务同一个目标——**让 microbatch 顶到 MFU 最优点**。
