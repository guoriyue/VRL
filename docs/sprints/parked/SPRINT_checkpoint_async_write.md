...
> 复用原语（实测存在,见 §4）：`vrl/generation/execution/worker.py:486-521`（pinned `non_blocking` D2H + 末尾一次 `cuda.synchronize()`,带 "~376ms cudaStreamSynchronize per chunk" perf 论证）、`vrl/generation/diffusion/pipeline.py:156-169`（`_move_tree_to_cpu_async`,专用 stream + event,不全局 sync）、`vrl/generation/ray/weight_sync.py:59` / `vrl/generation/ray/pipeline_runner.py:83`（`asyncio.to_thread` offload 阻塞调用）、`vrl/rollouts/orchestration/continuous/producer.py:77-90,109-143`（后台 task + `_inflight` + `stop()`/`drain_inflight()`）、`vrl/rollouts/orchestration/continuous/queue.py:19-37`（有界 deque + byte 预算）。
