...
> 复用原语（实测存在,见 §4）：`vrl/generation/ray/weight_sync.py:59` / `vrl/generation/ray/pipeline_runner.py:83`（`asyncio.to_thread` 是仓库唯一 offload 阻塞调用的方式）、`vrl/rollouts/orchestration/continuous/producer.py:77-90,109-143`（后台 `asyncio.Task` + `_inflight` set + `stop()`/`drain_inflight()` 生命周期）、`vrl/rollouts/orchestration/continuous/queue.py:19-37`（有界 deque + byte 预算 + 背压淘汰）、`vrl/generation/execution/worker.py:486-521`（pinned `non_blocking` D2H,带 perf 论证）。
