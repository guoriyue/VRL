...
不继承。三个 concrete pass：`QuantizationPass`、`CompilePass`、`TeaCachePass`。

**为什么 TeaCache 也进来**：它今天由 `DenoiseSamplingConfig.teacache` 驱动，
在 denoise loop 内部构造（`loop.py:135-137`），和量化/compile 走两条完全不同的路径。
它同样是「用精度换速度」的 rollout 优化，同样需要 drift guard 配套。
统一进 pass 注册表后，「本次 rollout 开了哪些 drift 源」有单一可查询答案 ——
这对 RL 至关重要（§3.4）。

注意 TeaCache 的挂载点与另外两个不同：量化/compile 改 module tree，TeaCache
改 denoise loop 行为。`TeaCachePass.apply` 不改 module，只把已解析的
`TeaCacheConfig` 登记到 bundle 上，由 loop 读取。这个差异要在 `PassResult`
上显式表达，不能假装三者同构。
...
它的消费者是 §4 P4 的 drift-guard 前置检查：**开了 drift 源却没 arm drift guard
的配置直接 fail loud**。今天这个检查不存在 —— TeaCache 的 docstring 只是用文字
警告「A run that enables TeaCache must arm the drift guard」，没有代码强制。
