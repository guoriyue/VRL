...
4. **`_cross_node_preflight` 归位**（发现 #9）：从 `launcher.py:337-362` 原样搬进 `vrl/ray/placement.py` 成公有 `cross_node_preflight`（概念同胞 `require_actor_gpu_ids` 旁）；launcher.py 删函数并从 dependencies import 里去掉 `inspect_cluster`；`scripts/common/online.py:742-744` 的跨包私有 deferred import 改顶层 `from vrl.ray.placement import cross_node_preflight`；测试搬到 `tests/ray/` 并重指；`dependencies.py:50` docstring 指针更新。
