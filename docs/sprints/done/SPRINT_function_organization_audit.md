...
| **ray** | lazy-Ray + actor-metadata utilities (dependencies.py); symmetric teardown pair `kill_actors`/`remove_placement_group` (lifecycle.py); shared placement utilities (placement.py); `RayActorJob` + state-owning `RayActorDispatcher` (actor_pool.py); the `resolve_distributed_resources` resolver pipeline + public derivation facades (resources.py) |
