...
**Stays free in `interfaces/replay.py`:** `validate_zero_replay_timestep` and `validate_replay_segments` — real non-AR callers exist: `causvid/model.py:447-448` (a `DiffusionModelBase` subclass) calls both, `janus_pro/model.py:361,366` calls both in its R1 branch, `denoise/base.py:175` calls one, and `tests/models/interfaces/test_replay_model_contract.py:116-140` unit-tests them with `owner="test"`. `single_segment_result` stays (pure wrapper, no subject).
...
|---|---|---|
| `require_chunk_gatherer` | ~~`generation/ray/utils.py:19`~~ **Superseded 2026-07-30** | The gatherer is now carried by typed `RayGenerationLaunchInputs`: that driver/wire boundary validates protocol, callability, and pickle safety, while `GenerationWorkerCore` independently revalidates the process boundary. The launcher-only helper became a third, weaker duplicate and was deleted. Do not restore it. |
| `_require_chunked_executor` | `generation/execution/worker.py` | Keep. Its input is the result of a dotted-string executor import, so this is the worker's real plugin boundary rather than type erosion. |
