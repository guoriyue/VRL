# SPRINT: Generation Regime 与 Policy Step 分层 — 方向裁决与层图固化

**Date**: 2026-07-13  **Status**: **DONE — taxonomy and layout landed (2026-07-18)**.
Chunk-autoregressive policy implementation is separately tracked and is not
remaining work in this decision sprint.
...
- **主仓位 ~80%**：chunk_autoregressive × denoise（CausVid 首接、MAGI-1 contract reference、
  exact chunk-wise Self-Forcing variants + provider 路线），
  兼顾 full-sequence denoise 试验台（sd3_5/flux/sana 是 RL 正确性的 CI，39/12/5 个
  真实 run）。
- **对冲 ~20%**：token-autoregressive categorical-token 策略保温不冷冻——token-GRPO 五家族保留；nextstep_1
...
full_sequence             │ current policies      │ （当前无）
token_autoregressive      │ （当前无）             │ current policies
chunk_autoregressive      │ CausVid/MAGI-1 等目标格 │ （未来可存在）
```

Generation regime、policy step、action distribution 与 trajectory layout 分开记录。
multisegment 是 trajectory layout，不是假装成第四种 generation regime。分类范围是
trainable policy，而不是整个 checkpoint：GLM-Image 是 token-autoregressive
categorical-token policy，后接
frozen full-sequence denoise renderer；Cosmos3 当前训练的是 full-sequence denoise vision stream。
...
vrl/generation/
  ...
  composition/token_autoregressive/   current ordered-prefix token state machine
  bindings/full_sequence_denoise/     full_sequence × denoise request/executor/gatherer
  bindings/token_autoregressive/      token_autoregressive × token request/executor/gatherer
...
当前只抽取了已经有多个真实 family 消费的 `composition/token_autoregressive/token_loop.py`。
Full-sequence stage orchestration 仍只有 `full_sequence_denoise` 一个真实 owner，因此
没有为对称性制造空 `composition/full_sequence.py`；`chunk_autoregressive` 也等首个
可执行 policy 再落。token
...
新 chunk-autoregressive family 应放 `vrl/models/families/<family>/`，并注册为
`(chunk_autoregressive, denoise, continuous, denoise)`；不要新建
`models/diffusion/<family>/`。候选的证据、顺序与 release gates 见
[`MODEL_TAXONOMY.md`](../../MODEL_TAXONOMY.md#future-chunk-autoregressive-support)。
...
3. **已完成：现有 composition 抽取。** token-autoregressive state machine 已进入
   `generation/composition/token_autoregressive/token_loop.py`；denoise hot loop 已进入
   `generation/steps/denoise/loop.py`。
4. **待真实实现：chunk-autoregressive denoise。** 首个技术候选是 Wan2.1-1.3B-based
...
- **ALL_CAPS**：`FAMILY_REGISTRY` 是刻意隔离的 taxonomy/config table；
  `GENERIC_FULL_SEQUENCE_DENOISE_EXECUTOR` 是跨 neutral registry/worker 的 import-path protocol，均
...
- registry mapping tests 覆盖现有兼容投影及 synthetic chunk-autoregressive denoise，防止 taxonomy
