...
- **LIVE RUN（关键，2026-06-19）**：真实 SD3.5-medium OCR GRPO，当前等价配置为
  `precision.training.dtype=bf16`（rollout base 继承 bf16）和
  `precision.rollout.quantization.format=fp8` + TIS，跑满 3 epoch 无崩。**首次 live run 抓到并修了一个真 bug**：rowwise `_scaled_mm` 不收 fp32 输出，而 DiT adaLN/pooled 路径喂 fp32 → `Fp8Linear` 改成 bf16 累加再 cast 回（commit 01dd8e7）。fp8 还和 **LoRA 共存**（337 个 base linear 在 adapter 里被 swap）。
...
- **门控**：FP8 ungate；the ambiguous generic `fp4` config token is rejected.
  The later NVFP4 implementation uses the precise `format: nvfp4` spelling.
...
[[SPRINT_fullparam_and_fp8_precision]] 落地了 fp8 的**精度-修正地基**（precision policy 的 sub-byte 词汇 + `rollout` 轴、drift guard 复用、TIS、验证 probe）。当时 fp8 rollout 还不能 live 跑；本 sprint 已把生成引擎 diffusion 前向接到真正的 fp8 GEMM。The current switch is
`precision.rollout.quantization.format=fp8`, layered on the ordinary
`precision.rollout.dtype`. FP8 is live; the later experimental NVFP4 path uses
`quantization.format: nvfp4`, while the ambiguous generic `fp4` spelling is
rejected. The family-builder fake-knob risk is closed by the backstop from
[[SPRINT_rollout_optimization_layer]] P0.
...
| 环节 | 文件:行 | 说明 |
|---|---|---|
| precision 入口 | `vrl/config/precision.py` + `vrl/models/model_build.py` | FP8 and experimental NVFP4 use `policy.rollout.dtype` as the ordinary base precision; `ModelBuild.rollout.quantization_format` selects the kernel policy |
| 模型加载（按 dtype） | `vrl/models/diffusion/sd3_5/model.py`（`from_build`）、`wan_2_1/model.py`、`cosmos/predict2/model.py` | 三家都按 `build.parameter_dtype` 加载 **diffusers transformer**；该值已在 model build 边界归一化 |
...
| 调用点（每家） | `sd3_5/runtime.py:56-78`、`wan_2_1/runtime.py`、`cosmos/predict2/runtime.py` | `from_build` → LoRA/full-finetune → `torch_compile`;fp8 swap 插在 **compile 之前**（LoRA 之后）|
...
3. ✅ **Parameter dtype split and FP8 enablement**: the ordinary rollout
   parameter/autocast precision remains the bf16/fp16/fp32 value in
   `policy.rollout.dtype`; FP8 replaces only selected large GEMMs. Experimental
   NVFP4 uses the precise `format: nvfp4` spelling and MLP-only targeting. The public
   `precision.rollout.quantization` block projects to
   `ModelBuild.rollout.quantization_format`; rollout builders apply the swap
   before compile, while replay builders never apply it.
4. ✅ **scaling recipe + 精度 profile**：rowwise（默认）+ tensorwise 实现并验。已退役的 `vrl/scripts/perf/fp8_recipe_accuracy.py`（fake-quant，对齐过真 `_scaled_mm`）留下的历史四档漂移：
...
- **绝不碰**：SDE / logprob 数学（`sde_step_with_logprob` 等；当前字段
  `precision.diffusion_math.dtype` 默认 fp32）。
...
4. **Guard**: an FP8 live run uses
   ...
   can no longer silently ignore the request and run plain bf16.
