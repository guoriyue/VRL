...
>   - **runtime capability 派生**：`RayGenerationLauncher._launch_session` 通过 `_all_workers_support_versioned_slots(ray, workers, weight_sync=...)` 把 `RayGenerationSession.supports_non_draining_weight_sync` **派生为所有 worker `supports_versioned_trainable_state()` 的 AND**。eager runtime 在构造时向 schedule 暴露该能力；deferred runtime 在 session 尚未创建时保持保守 `False`，activation 后也继续走 draining fallback。`weight_sync is None`（sync 关闭）/ 无 worker / 任一 query 明确返回 False → False → 安全回退 draining barrier；query RPC 抛错表示 candidate session 已坏，必须上抛并由 launcher 清理 actors，不能伪装成“不支持”。
...
>   - **CPU 测试**：`test_runtime_config.py`（AND 派生 + 无 sync/无 worker/明确 False 安全回退；query exception 上抛并清理 candidate fleet）、`test_oom_split.py`（stale-slot raise `StaleSlotDiscard` 且不重试、非 RuntimeError 子类）、`test_schedule.py`（StaleSlotDiscard → discarded_stale 而非 error_count + queue 保持空）。全相关套 47 passed；`tests/generation/` + `tests/rollouts/orchestration/` 137 passed / 2 skipped 无回归。
> - **P2 多卡验收已完成**：2 卡分池验证了无 policy-version mismatch、non-draining
>   weight sync 不再等待整条 clip，并记录了每步 wall-clock 改善；完整结果见 §11。
