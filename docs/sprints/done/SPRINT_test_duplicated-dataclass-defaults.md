...
> Precision naming note (2026-07-12): the code excerpts below preserve the
> historical flat `precision.train`/`precision.rollout` policy used by that
> sprint. Current code uses `policy.training`/`policy.rollout` role objects and
> selects FP8 through `policy.rollout.quantization`. The excerpts are evidence,
> not current configuration or implementation guidance.
