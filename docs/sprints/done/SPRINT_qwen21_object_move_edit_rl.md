...
## 15. 第二轮准备（2026-09-23）：稠密奖励 + 合理指令 + base 关卡

run1 之后按 §12/§14 改了信号和数据（commits 7d9ef6db、ed9a1438、58476176、d8d7f2dc、b613d674、ce44ad01、92a8e732）：

- **奖励**：`object_move_shaped = (object_move + 0.2·background) / 1.2`，复制 / 没动 / 丢失只要没重画背景都有 0.167 底分；训练键改为 shaped。
  去掉 left/right/closer/farther 模式，只留 `support_box`（"放到 X 上"：物体底边中心落进承托物框，按缩短的距离给部分分，深度缩放 4x 内不扣）和
  `target_box`（SpatialEdit 红框，IoU）。身份阈值在 support 模式降到 0.15（放到新位置常换个角度），家具大小的噪声框不计入个数。
- **数据**：COCO 只取可移动类别 + 图里真实存在的承托物（椅子 / 沙发 / 床 / 餐桌），规则筛（物体 0.8–12% 画面、承托物 ≥8%、不已在承托物上、无人遮挡）
  后 498 条交 Opus 逐条审"这个移动在这张图里合理吗"，通过 264 条；指令 "Move the {name} from the {origin} onto the {support}."。
  训练集 720 = COCO 203 + SpatialEdit 517；held-out COCO 61（按图片划分）+ SpatialEdit-Bench 64。
- **run2**（旧指令 + shaped 奖励）跑了 17 轮后应用户要求停掉，换成新数据重开 run3；不作结论。
- **base 关卡（训练采样设置 512 px / 10 步 / SDE，48 条训练指令 × 4）**：检测器分桶 base 移动 45%；抽 80 张交 Opus 盲判：**移对 24（30%）、复制 43（54%）、
  没动 11、放错 2** → 落在 20–60%，通过。修复前的奖励只认出 6% 成功（漏掉换了视角的正确移动、把家具当成第二个物体），修复后与 Opus 一致 74/80。
  盲判还确认：复制率在简单 / 复杂场景里都是一半左右（≤6 个物体且物体 ≥3% 画面：50%；其余 54%），**大物体更容易被复制**（≥3%：65%，<3%：42%）——
  难点是"擦掉原物体并补好洞"，不是"找到物体"；换简单的图解决不了复制。奖励与 Opus 的一致率在简单 / 复杂场景都在九成上下，场景复杂度不影响打分。

## 16. run3（2026-09-23 17:32 → 09-24 01:10）

启动（tmux `om_run3`，等 GPU 空闲后执行；20:53 因手动停机一次后由 supervise 从 checkpoint-20 恢复）：

```bash
python -m vrl.scripts.supervise --config experiment/qwen_image_21/online_grpo_object_move \
  --max-attempts 6 --health-metrics --health-max-grad-norm 0.5 \
  model.lora.parameter_dtype=float32 precision.float32_precision=ieee \
  trainer.output_dir=outputs/qwen_image_21_object_move_run3 trainer.total_epochs=60 trainer.save_freq=20 \
  trainer.debug.first_step=true
```

replay parity PASS（max abs diff 2.5e-4，上限 1e-2）；生成阶段峰值 20.4 GB；单次更新约 6.5 min；54 轮共约 7.4 h 有效训练。

| update | reward_mean | reward_std | clip 均值 / 最大 | kl_penalty | grad_norm | adv_zero |
|---|---|---|---|---|---|---|
| 1–10 | 0.148 | 0.144 | 0.087 / 0.129 | 0.0001 | 0.0004 | 0.00 |
| 11–20 | 0.124 | 0.109 | 0.091 / 0.113 | 0.0005 | 0.0005 | 0.00 |
| 21–30 | 0.142 | 0.134 | 0.099 / 0.129 | 0.0008 | 0.0011 | 0.03 |
| 31–40 | 0.092 | 0.089 | 0.088 / 0.150 | 0.0012 | 0.0010 | 0.07 |
| 41–50 | 0.078 | 0.088 | 0.098 / 0.179 | 0.0015 | 0.0012 | 0.02 |
| 51–54 | 0.131 | 0.083 | 0.161 / **0.204** | 0.0022 | 0.0015 | 0.00 |

奖励 60 轮内没有上升趋势；第 37–40 轮跌到 0.071 / 0.070 / 0.013 / **0.0004**（整批 48 张背景项≈0），之后回到 0.05–0.20；clip 从第 52 轮起 0.14 → 0.19 → 0.20，
第 54 轮 0.204 越过 0.2 停止线，**按规则 SIGTERM 停机**（先读 cmdline 确认，先停 supervisor 再停 trainer），没有 checkpoint-60。

第 20 轮中途一瞥（held-out 前 8 条，同噪声）没有发现重画；但发现打分器会冤枉正确编辑：箱子从草地放到沙发上，几何 1.0、身份 0.72，
**背景项只有 0.06**（base 只挪了一点：0.49），总分 0.22 < base 0.40。像素差显示背景几乎没变（墙 3.3、路面 9.0，与 base 相当），
是 EfficientLoFTR 在沙发绒布 / 柏油路这类**单调纹理上错配**（沙发区域 1053 个匹配只剩 67 个"原位"）。按"不中途调参"未改。

## 17. Verdict：ck20 持平，ck40 BROKE

held-out 61 条 COCO（1024 px、40 步 ODE、seed 0、各 arm 同噪声），base / ck20 / ck40 共 183 张，**8 个独立 Opus 判官盲评**（arm 打乱隐藏）为主判据；
奖励与检测器分桶为辅。输出 `outputs/qwen_image_21_object_move_run3/verdict/`，标签在会话 scratchpad `verdict3/labels/`。

| 集合 · 指标 | base | ck20 | ck40 | Δ（95% bootstrap CI） |
|---|---|---|---|---|
| COCO 61 · 盲评 移对 / 复制 / 没动 / 放错 / **重画** | 20 / 61 / 16 / 3 / 0 % | 25 / 56 / 16 / 3 / 0 % | 0 / 2 / 0 / 0 / **98** % | |
| COCO · base↔ck20 逐条配对 | | 变好 5、变差 2、都对 10、都不对 44 | | 净 +3 / 61，与噪声无法区分 |
| COCO · reward（object_move） | 0.141 | 0.116 | 0.003 | ck20 −0.024（−0.072, +0.024）；ck40 −0.138（−0.203, −0.083） |
| COCO · 奖励与盲评一致率 | 54/61 | 55/61 | 61/61 | |
| SpatialEdit-Bench 64 · reward | 0.312 | — | 0.009 | −0.302（−0.366, −0.240） |
| SpatialEdit-Bench · benchmark overall / IoU / Score_oc | 0.661 / 0.724 / 0.702 | — | 0.142 / 0.115 / 0.631 | overall −0.519（−0.595, −0.441） |
| SpatialEdit-Bench · 检测器分桶 移动 / 不动 / 复制 / 丢失 | 72 / 6 / 16 / 6 % | — | 48 / 11 / 5 / **36** % | |
| 镜头类 14 条 · 清晰度 | 0.627 | — | 0.632 | +0.005（−0.062, +0.065） |

- **ck20**：与 base 几乎同一个模型——盲评 49/61 条结论相同，移对 12 → 15，复制 37 → 34；奖励均值略降但 CI 跨 0。判**持平**。
- **ck40**：60/61 张不是编辑，是把源图重画成拼贴 / 相框里的照片，构图全变（对比页第四列）。判**BROKE**，与 run1 同一种坏法，同样出现在 40 轮附近。
  镜头类清晰度指标没抓到退化（同 run1），以目测为准：ck40 的所有输出都重画，镜头编辑也不可能幸免。
- 时间线：训练奖励在第 37–40 轮跌到 0，就是滑进"重画"模式的时刻；之后训练奖励回到 0.1–0.2 是在 512 px / 10 步 SDE 下量的，
  1024 / 40 步 ODE 下 ck40 已全是重画。

对比页（16 组：ck20 变好的 5、变差的 2、都对的 4、随机 5；每张带盲评判定、备注、奖励分）：<https://claude.ai/artifact/5Xm79LcnECucQ8eZnqSu3r>
训练前 80 张盲评（复制 / 移动分栏，红框原位置、蓝框承托物）：<https://claude.ai/artifact/HF9T6HEpJAmP2G8iGhAH4u>

**目标"held-out 移动成功率提高"未达成。** 两次 run 的共同点：训练奖励在崩之前没有上升；信号本身给不出稳定的上升方向——
(1) 正确移动经常被背景项冤枉（§16 的箱子），(2) 复制和没动拿一样的底分，模型没有"擦掉原物体"的动力，(3) 一旦随机漂进重画，背景项对所有样本都是 0，
组内优势归零，没有拉回来的梯度。**下一轮先修奖励、离线验证，再谈训练**：
1. 背景项换掉特征匹配——用编辑区域之外的像素 / DINOv2 局部特征相似度，同样能抓整图平移和重画，不会被单调纹理骗；
2. "原物体从原位置消失"单独计分，把复制和真移动拉开；
3. 在本 sprint 攒下的 80 + 183 张 Opus 标签上验证新奖励的一致率和排序，先过 GATE A 再开 run4。

## 18. 交付物（run3）

- Commits（未 push）：7d9ef6db、ed9a1438、58476176、d8d7f2dc、b613d674、ce44ad01、92a8e732、152ff548、本文档。
- 输出：`outputs/qwen_image_21_object_move_run3/`（checkpoint-20/40、metrics.csv、peek_ck20/、verdict/）、`outputs/qwen_image_21_object_move_base/base_rate_512`（base 关卡）。
- 盲评标签、渲染 / 聚合脚本、对比页源码在会话 scratchpad `verdict3/`；训练前 80 张标签在 `onto/label/`。
- 平行探索（动漫 / 真人肖像还能练什么）见 `docs/sprints/planned/SPRINT_qwen21_accessory_edit_rl.md` §7。

## 19. 奖励修复（2026-09-24，离线，未训练）：背景项换成 DINOv2 分块一致率 × 相位相关位移检查

依据 §16 的箱子案例，在 183 张盲评图上对照了三种"背景没变"的度量（会话 scratchpad `verdict3/background_study.py`）：

| 度量 | 保住 vs 重画 AUC | 正确移动的取值（min/p25/中位/p75/max） | 源图平移 3% | 换一张场景 |
|---|---|---|---|---|
| 旧：EfficientLoFTR 原位匹配份额 × 覆盖率 | 1.000 | 0.06 / 0.42 / 0.56 / 0.78 / 0.92 | 0（设计目的） | 0 |
| DINOv2 分块余弦 > 0.5 的份额（编辑框之外） | 1.000 | 0.75 / 0.87 / 0.90 / 0.93 / 0.95 | 0.72–0.91（看不出平移） | 0–0.16 |
| 相位相关整体位移（灰度 256 px） | — | 107/108 张保住的 ≤1% | 全部 2.7–3.1% | 全部 >3% |

新背景项 = 分块一致率 × 位移检查（位移在 `stay_tolerance`=1% 对角线内满分，2% 归零；相位相关前把编辑框连同 2% 余量抹掉，
否则平坦场景里的大物体会独占相关峰）。EfficientLoFTR 整个去掉。Commit 见本节末尾；单元测试改成带纹理的合成场景，位移检查走真实代码（13 个通过）。

用最终代码重打 183 + 80 张有 Opus 标签的图（CPU，`verdict3/validate_new_reward.py`）：

| | 与盲评一致率（旧 → 新） | 正确移动的分数中位（旧 → 新） | 重画的分数 | 背景项为 0 的非重画图 |
|---|---|---|---|---|
| held-out base / ck20 / ck40 | 54→54、55→**58**、61→61（/61） | 0.46 → **0.64**（p25 0.21 → 0.48） | 最大 0.16 | 1（ck40 一张场景已改的复制） |
| 训练前 80 张（512 px） | 74 → 74（/80） | 0.52 → 0.59 | — | 0 |

shaped 键的排序：正确移动 > 忠实失败（复制 / 没动）AUC 0.855（训练前集 0.888），忠实失败 > 重画 0.980。那对箱子：0.22 → 0.74。

仍未解决、留给下一步的：复制和没动仍拿同样的 0.167 底分，模型没有"擦掉原物体"的动力——需要"原物体从原位置消失"单独计分（同样先在这两套标签上离线验证）。
run4 未启动。

## 20. 奖励修复（二）：底分只在"原位置已空"时全给

复制（base 输出的 54%）= 放了新的、没擦旧的；之前复制和没动同拿 0.167 底分，没有任何梯度指向"擦掉原物体"。
新增 `object_move_vacated`：编辑图里没有任何归属于该物体的实例仍与源框重叠（IoU ≥ 0.5）。
`object_move_shaped = (object_move + w·background·(1 + vacated)/2) / (1 + w)`：擦了没放（丢失 / 放错地方）拿满底分 0.167，复制 / 没动拿 0.083，
`object_move` 本身不变。单元测试 13 个通过。

在两套 Opus 标签上验证（`verdict3/validate_new_reward.py`，v3）：

| | vacated 与判官"原物体还在"一致 | 正确移动被判 vacated | 复制被误判 vacated | shaped 排序 移对 > 复制/没动（旧 → 新） | 擦了没放 > 复制/没动 |
|---|---|---|---|---|---|
| held-out 183（去掉重画 60） | 107 / 123 | 27 / 27 | 13 / 72 | 0.855 → **0.953** | 0.856 |
| 训练前 80 | 76 / 80 | 23 / 24 | 2 / 43 | 0.888 → **0.950** | 1.000 |

各类 shaped 中位：移对 0.69 / 0.65，擦了没放 0.16 / 0.60，复制 0.08 / 0.08，没动 0.14 / 0.20，重画 0.00。
误判的 13 张复制多是检测器在编辑图里漏掉了留在原位的那个（拿到满底分而不是一半，影响轻微）。

至此 §17 列出的三条修法（背景项、原位置项、离线验证）都已完成并提交（f43bc300、本节 commit）。**run4 未启动，等授权**；启动时沿用 §16 的命令，
训练键仍是 `object_move_shaped`，其余不变（不是超参问题）。

## 21. run4（2026-09-24 10:16 起）：修过的奖励，其余与 run3 相同 —— 第 20 轮 held-out 学到了

唯一改动是 §19–20 的奖励（commits f43bc300、2366951b）；数据、配方、超参与 run3 完全一样。启动命令同 §16（输出目录 run4）。
启动关卡：replay parity 2.4e-7 通过；grad_norm 2.9e-4；生成峰值 20.3 GB；单轮约 8 min（DINOv2 分块多两次前向）。

训练奖励（新奖励下训练前基准 0.262）：1–10 轮 0.318、11–20 轮 0.267、21–30 轮 0.290、31–36 轮 0.292；clip 最大 0.14，KL 0.0009。
**曲线看起来平，是分辨率问题**：每轮 6 条新指令，噪声 ±0.05，而真实提升约 +0.1；以后看 20 轮平均。
第 36 轮时用户的 `kaudit` 内核审计在同一张卡上瞬时占 3–9 GB，run4 峰值 26 GB 之上连续两次 OOM，supervise 熔断停机（不是训练故障）。
16:50 从 checkpoint-20 续跑（gpu_retry 包装：等显存 ≥28 GB 再起、失败重试）。

**checkpoint-20 的 held-out 判定（61 条 COCO，同噪声）：**

| | 原始 | run4-ck20 | 配对差（95% CI） |
|---|---|---|---|
| 1024 px / 40 步 · object_move | 0.159 | **0.238** | **+0.080（+0.014, +0.149）**，赢 36% 平 43% |
| 512 px / 10 步 × 4 样本（244 对）· object_move | 0.113 | **0.224** | **+0.112（+0.069, +0.159）**，赢 56% |
| 512 × 4 · 成功率（>0.3） | 17.2% | 34.0% | |
| 1024 · **盲评** 移对 / 复制 / 没动 / 放错 / 重画 | 11 / 40 / 8 / 2 / 0（18% / 66% / 13% / 3% / 0） | **21** / 24 / 13 / 3 / **0**（34% / 39% / 21% / 5% / 0） | 逐条配对：**变好 10、变差 0**、都对 11、都不对 40 |
| 奖励与盲评一致率 | 53 / 61 | 49 / 61 | |

8 个独立 Opus 判官盲评（arm 打乱隐藏，标签在 scratchpad `verdict3/labels4/`）。原始模型这次的标签与 run3 评测时的原始标签一致 57/61（同一批图，判官独立），说明判官稳定。
复制 40 → 24，"没动"略升 8 → 13（模型有时学会了"不复制"但没放到位），没有任何重画。判定：**ck20 学到了**。
对比页（全部 10 条变好 + 3 条都对 + 3 条都没对）：<https://claude.ai/artifact/AGRP3kgHKsU5nxTNKBLqT6>

待补：ck40 / ck60（续跑中，约 22:00 出 ck60，评测脚本 `om_verdict4` 已就位：COCO 1024、SpatialEdit-Bench、相机回归、512×4）。

## 22. 奖励审计：它有多"启发式"，以及 VLM 判官能不能替它（2026-09-24）

**审计结论**：测量用的是现成模型（Grounding DINO、DINOv2、相位相关），但"测量 → 分数"是约 10 个手工决定（检测/NMS 阈值、身份匹配下限 0.5 / support 模式 0.15、
尺寸容忍 4x–8x、"底边中点落进家具框 = 放上去"、原位置 IoU ≥ 0.5、分块余弦 ≥ 0.5、位移 1%/2%、开方乘积、底分 0.2 与减半）。
每条都在 263 张独立 Opus 标签上验证过（§19–20），但它们相互耦合（一个身份阈值同时决定计数、原位置、背景排除区域），
且"家具框 ≠ 可放置面"（椅背也在框里）。第二位审阅者的意见与此一致：适合做受限任务的诊断工具，不宜当通用"移动是否成功"的唯一裁判；
应先定义 `reward(source, edit, instruction)`、建独立测试集、独立比较通用打分器，再让失败案例驱动改进，而不是加例外。

**VLM 判官对照**（`verdict3/vlm_judge_study.py`，Qwen2.5-VL-7B，两张 448 px 图 + 四个是非题取 yes 概率的几何平均，CPU，263 张）：

| | held-out 183：移对 > 其余 AUC | 移对 > 复制/没动 | 复制/没动 > 重画 | 与盲评一致率 | 训练前 80：移对 > 其余 | 一致率 |
|---|---|---|---|---|---|---|
| VLM 判官 | 0.755 | 0.721 | 0.603 | 157/183 | 0.722 | 59/80 |
| object_move | 0.908 | 0.902 | 0.550 | 173/183 | 0.914 | 74/80 |
| object_move_shaped | 0.967 | 0.953 | 0.955 | — | 0.940 | — |

分问题看（yes 概率中位数，移对 / 复制 / 没动 / 重画）："放在 Y 上了吗" 0.99 / 0.96 / 0.13 / 0.35；"原位置没了吗" 0.59 / 0.49 / 0.52 / 0.71；
"只有一个吗" 0.78 / 0.59 / 0.49 / 0.69；"其余没变吗" 0.75 / 0.68 / 0.68 / 0.36。
它**分不清复制和移动**（本任务最主要的失败），对重画的识别也弱。零样本 7B VLM 看两张小图数不清物体、记不住原位置。

**决定**：训练奖励维持现状（最可信的信号，run4 第一次学到），不再加规则、不动 0.15。更可信裁判的候选按顺序：
(1) EditReward（仓库服务，`verdict3/editreward_study.py`，排在 run4 评测后）；(2) Opus 作在线判官（判官间一致 57/61，是唯一被证明可靠的；成本约每轮 96 次调用）；
(3) 7B VLM 的补救（放大裁剪、先定位再问、更大的 VL 模型）——记录，不在本 sprint 做。

## 23. run4 收尾（2026-09-24 19:56，用户叫停）

- 时间线：10:16 启动 → 14:52 第 36 轮被显存 OOM 熔断（用户的 `kaudit` 内核审计在同一张卡上瞬时占 3–9 GB）→ 16:49 从 ck20 续跑 → 19:31 存 ck40 →
  19:45 第 41 轮被**主机内核 OOM** 杀掉（训练进程常驻内存 77 GB / 91 GB，续跑后每轮涨约 2 GB；原因待查，候选：休眠换出到 CPU 的奖励/模型状态、
  跨轮累积的回放或奖励记录）→ 19:47 从 ck40 再续（存档间隔改 10）→ 19:56 用户叫停，SIGTERM 后进程自行收尾退出。
- 有效训练 41 轮（含重跑的 21–36），单轮约 8 min，生成峰值 20.3 GB，parity 2.4e-7。
- 交付状态：ck20 的 held-out 判定完整（§21：学到了，盲评移对 18% → 34%，10 变好 / 0 变差，奖励 +0.080 CI 不跨 0，页面 <https://claude.ai/artifact/AGRP3kgHKsU5nxTNKBLqT6>）；
  **ck40 未评、ck60 不存在**，SpatialEdit-Bench 与相机回归检查未跑（评测链 `om_verdict4` 仍待命，可对 ck20/ck40 触发）。
  本 sprint 的判定以 ck20 为准：**学到了**；目标"held-out 移动成功率提高"在 ck20 上成立，更多轮次是否继续提升未验证。
- 待办（不需 GPU 的先做）：训练进程主机内存增长的根因；EditReward 三方对照（`verdict3/editreward_study.py`，需服务唤醒 16 GB 显存）。
