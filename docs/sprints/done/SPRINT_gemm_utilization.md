...
用 `vrl/scripts/perf/profile_linear_projections.py`（config-init 随机权重 + 生产维度；GEMM 时间由 shape 决定，故分布忠实）实测 per-projection GEMM **CUDA self-time** 占比（batch=2 = cond+uncond，eager，LoRA off，单次前向；wan 用 12 层、生产宽度，比例与层数无关）：
