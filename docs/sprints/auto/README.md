...
注：已移除完成项；下表统计的是仍待处理的 open sprint 文件。
...
| verdict | 数量 | 含义 |
|---|---|---|
| delete | 1 | grep 确认无引用的死代码，可删 |
| consolidate | 1 | 重复/可合并到别处 |
| question | 5 | 存在性存疑，需拍板 |
| improve | 38 | 真核心但有具体改进点（命名/拆分/derive/裁字段）|
| keep-justified / resolved | 217 | 审过无问题、未写 sprint，或已完成并移除 sprint |
...
- **`vrl/trainers/online/__init__.py`** (17 LOC, role=package-init) — vrl/trainers/online/__init__.py — 私有 _validate_ema_state_shapes 仍进 __all__，外部只用 OnlineTrainer，应收窄
