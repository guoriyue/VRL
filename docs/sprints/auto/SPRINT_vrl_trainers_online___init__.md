...
## 0. 一句话
package init 仍把私有 `_validate_ema_state_shapes` 提升进 public `__all__`，但外部只消费 `OnlineTrainer`——应收窄到只导出 `OnlineTrainer`。

## 1. 现状（读代码得出）
```python
from vrl.trainers.online.trainer import OnlineTrainer, _validate_ema_state_shapes
__all__ = ["OnlineTrainer", "_validate_ema_state_shapes"]
```

## 2. 质疑点 / 改进机会
- 实际通过 `from vrl.trainers.online import X` 消费的只有 `OnlineTrainer`（`vrl/trainers/__init__.py`、`vrl/scripts/common/online.py`、`tests/...`）。grep `from vrl.trainers.online import` 全树没有任何条目导入 `_validate_ema_state_shapes`。
- `_validate_ema_state_shapes` 是 trainer.py 内部 helper，仅被 `trainer.load_state_dict` 调用；它带下划线前缀却被列进 `__all__`，语义矛盾，且无外部消费者。
...
- 把整个文件收窄为：
  ...
  ```
- 删除对 `_validate_ema_state_shapes` 的 re-export。
