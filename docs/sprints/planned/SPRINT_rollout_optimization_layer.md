...
1. **config-wire fp8 recipe**：`blockwise`（reuse vLLM triton，更快更准、显存更多）现在只能 API 开（`recipe="blockwise"`）。接一个 config 旋钮让 rollout 能选 recipe（默认 `rowwise`，blockwise opt-in，显存够时用）。
