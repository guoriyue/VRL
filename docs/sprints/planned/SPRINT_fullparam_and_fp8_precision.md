...
- **待接（解冻 FP8 真正剩的活）**：rollout 引擎的 diffusion 前向真正发 fp8 `_scaled_mm`（`vrl/generation/` + 自研/torchao GEMM 路径），让 `precision.rollout=fp8` 不只是策略 token 而真的省 GEMM 时间。这是 kernel 工程，受推理路径门控。
