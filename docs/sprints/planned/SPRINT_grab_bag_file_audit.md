...
9. **AR/Diffusion layout 三件套收敛**（发现 #8）：`vrl/generation/execution/chunks.py`（既有 stateless 之家）加 `require_chunk_rows`、`ordered_covering_chunks(..., row_fields, family)`、`max_peak_memory_mb`；AR/Diffusion 两个 layout 类公共签名不变、各成一行 delegate（diffusion 传 `row_fields=("observations","actions","log_probs","timesteps","kl","video")`）；删 diffusion `_require_rows`。唯一行为中性差异：diffusion coverage 报错串多一个 "sample" 词（无断言）。**落地前与记忆里 in-flight 的 diffusion layout.py reconciliation 协调（先 git fetch/status）。**
