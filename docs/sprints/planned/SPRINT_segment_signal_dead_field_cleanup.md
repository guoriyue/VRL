...
`fields(signal)` 会枚举 `SegmentSignal` 的**所有**字段，但 `getattr(...) is not None` 过滤器把永远为 `None` 的字段直接滤掉。结合「只有 `GRPO` 在 `vrl/algorithms/grpo/continuous.py:47` 直接声明 `required_signal_keys = ("log_prob", "old_log_prob")`，`TokenGRPO(GRPO)` / `MultiSegmentTokenGRPO(TokenGRPO)` 继承同一组 signal 契约，其余非 GRPO 算法继承 `base.py:33` 的空元组」这一事实，得出三档判定：

1. **永远 `None` 的字段**（`entropy`）→ 被 `is not None` 滤掉，永远进不了 `available`，永远不影响 `missing`，永远不改任何报错。审计中「通过 `fields()` 反射存活」的翻案对它是**机械性错误**。→ **删**。
2. **永远非 `None` 但只进 `available` 诊断串、自身从不被任何 `required_signal_keys` 要求、也无按值读取**（`axes`、`aux`）→ 唯一读者是诊断构造，按 AGENTS.md 派生结构体规则属 display-only 死字段。→ **删**；未来若要 per-segment 日志，应以真实日志消费者重新引入，而不是保留无消费者字段。
...
合证：`entropy` 永远 `None` → 被 `vrl/algorithms/trajectory.py:55` 的 `is not None` 滤除 → 永不进 `available`；任何算法的 `required_signal_keys` 都不含它；零 `.entropy` 按值读取。它是一条**半成品信号链**：占位字段 + 占位开关，两端都没接。

### 1.2 `SegmentSignal.aux` —— confirmed dead（写入了真元数据，但零消费者）
...
与 `entropy` 的区别：`aux` 被填了**有意义的元数据**，像是给未来 per-segment 日志预留的钩子，只是日志构造从未写出。按 AGENTS.md「只有实际日志/诊断消费者才算 display/provenance-only」的规则，当前不能仅因未来意图保留；本 sprint 直接删除。未来若需要逐段日志，届时带着真实日志消费者重新引入。
...
- **唯一「读者」** 是 `vrl/algorithms/trajectory.py:55` 的 `fields()` 反射 —— `axes` 永远非 `None`，故永远落进 `available` 列表，但 `available` 只在**别的** required key 缺失时拼进报错串 `Available signal keys: [...]`。`axes` 自身从不被任何 `required_signal_keys` 要求，进 `available` 不改任何控制流。
...
| `distribution` | `types.py:16` | `continuous.py:142` `== "flow_matching"`；`types.py:56-59` 非空校验 |
...
| `SignalRequest.need_ref` | `types.py:86` | `sde_logprob.py:103`；`ar/token_logprob.py:59`；`ar/continuous_token_logprob.py:53`；`multi_segment_token_logprob.py:65` |
...
- 删 builder 形参与穿透：`vrl/rollouts/evaluators/trajectory.py:44,63,89,146` 的 `aux` 形参/传参/填充。
- 删 `vrl/rollouts/evaluators/ar/multi_segment_token_logprob.py:93-96` 的 `aux={...}` 填充。
- 不保留 display-only 标注：当前没有真实日志/诊断消费者；未来 per-segment 日志若需要这些元数据，应随日志消费者重新引入字段或直接从 `TrajectorySegment` 派生。
