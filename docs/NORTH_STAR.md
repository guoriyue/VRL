...
**visual-rl = 视觉生成式 RL 的专用引擎**：joint-denoise、causal-token 与未来
causal-chunked video/world-model policy 的 RL 训练，在这里**一份 config 就能正确跑通、
跨模型族、且结果可复现**。这里的分类对象是 trainable policy，不是 checkpoint 名称；
完整定义见 [`MODEL_TAXONOMY.md`](MODEL_TAXONOMY.md)。
...
| **visual-rl** | **"视觉生成式 RL 是一个独立的系统+正确性问题（异构时间组织/step + 视觉 action log-prob + 像素奖励 + rollout↔train 共卡），LLM-RL 框架结构上变不成它"** |
...
| 单一 causal categorical-token 范式 | **正交 policy 轴**：joint/causal/causal_chunked × denoise/token；action distribution 与 trajectory layout 另列 |
...
1. **去噪轨迹 rollout 编排** — `vrl/rollouts/orchestration/continuous/`（producer/queue/consumer + staleness）+ `vrl/generation/steps/denoise/loop.py`（逐步去噪、每步产出 `log_prob/prev_sample_mean/std_dev_t`）+ `vrl/generation/bindings/joint_denoise/`（request/trajectory/decode）+ `vrl/trainers/weight_sync.py`（version-stamped 权重同步）。多步轨迹捕获 + 每步 replay 重算 old_log_prob，是 causal-token rollout 没有的形状。
2. **统一模型族契约** — `vrl/models/interfaces/replay.py`（`RuntimeModel.replay_forward`）+
   ...
   joint-denoise + 6 个 causal-token，含任务变体）**，都藏在同一个 replay 契约后面。
...
| 级别 | gap | 为什么是这个级别 |
|---|---|---|
| **P0 信任** | **只有一条 validated reference 曲线** | SD3.5-OCR 已证明一条路径，但还不足以证明 video joint-denoise 与 causal-token image 两种 policy profile。采用仍被“reward 是否真的能涨”阻塞。 |
| **P0 上手** | **quickstart 尚未做到轻量可复现** | README 已有首个 SD3.5-OCR job 和预期信号，但仍需要真实权重、OCR 依赖和 GPU；离 clone 后快速验证完整 RL 闭环仍有距离。 |
| **P0 广度** | **23 个 registry entry 只有 1 个 validated family** | 每个 validated profile 都会扩大可信覆盖，但不要陷入逐 entry 跑曲线的跑步机——先钉 3 条（image joint-denoise / video joint-denoise / causal-token image）作为信任锚。 |
...
### 阶段 A —— 信任层(P0,最高优先)
- **3 条 reference 曲线**（image joint-denoise=SD3.5/Flux、video joint-denoise=Cosmos/Wan、causal-token image=Janus），每条：固定 prompt 集 + BLOCK 测试判显著性 + 一键复现命令 + reward-vs-epoch 的 csv。归并现有 `SPRINT_cosmos_predict2_2b_trustworthy_curve` / flux algo validation。
...
3. **加族成本**：加一个新 family 到能跑 RL < 1 天，只动 `models/families/<family>`、descriptor、presets/tests，不动 trainer/algorithm。（今天：已有指南与 descriptor seam，但没有由 CI 验证的可复制 skeleton。）
...
**TL;DR**：别做 vLLM 的推理服务克隆。做**视觉生成式 RL 的 DeepSpeed**——押“这是个独立的正确性+系统问题”。护城河（去噪 rollout / 统一族契约 / denoise RL 数学）和首个 quickstart/加族指南已经有了；下一步是**补齐三种 policy profile 的可信曲线、做轻量可复现 quickstart、把指南变成 CI 验证的 skeleton**。性能别当卖点（已近 MFU 顶），正确性和覆盖才是。
