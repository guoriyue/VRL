...
**visual-rl = 视觉生成式 RL 的专用引擎**：full-sequence denoise、token-autoregressive 与未来
chunk-autoregressive video/world-model policy 的 RL 训练，在这里**一份 config 就能正确跑通、
...
| **visual-rl** | **"视觉生成式 RL 是一个独立的系统+正确性问题（异构 generation regime/step + 视觉 action log-prob + 像素奖励 + rollout↔train 共卡），LLM-RL 框架结构上变不成它"** |
...
| 单一 causal categorical-token 范式 | **显式 policy 语义**：full_sequence/token_autoregressive/chunk_autoregressive × denoise/token；action distribution 与 trajectory layout 另列 |
...
### 阶段 A —— 信任层(P0,最高优先)
- **3 条 reference 曲线**（image full-sequence denoise=SD3.5/Flux、video full-sequence denoise=Cosmos/Wan、token-autoregressive image=Janus），每条：固定 prompt 集 + BLOCK 测试判显著性 + 一键复现命令 + reward-vs-epoch 的 csv。归并现有 `SPRINT_cosmos_predict2_2b_trustworthy_curve` / flux algo validation。
