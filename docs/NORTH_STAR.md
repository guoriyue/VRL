...
2. **统一模型族契约** — `vrl/models/interfaces/runtime.py`(`RuntimeModel.replay_forward`)+ `vrl/rollouts/families/registry.py`。10+ diffusion 族(flux/sd3_5/wan/cosmos/qwen/echo/anima)+ 3 AR 族(janus_pro/janus_r1/nextstep_1)藏在一个 `replay_forward(batch, timestep_idx)` 后面,**加一个族 = 注册表一行**,trainer/algorithm 零改动。
...
| **P0 上手** | **没有可执行的 quickstart** | "5 分钟跑通第一个 RL job"是让人不去 hack slime 的关键。现状:README 有 setup 无"run your first job",最小 config 叠 8+ 层。 |
| **P0 广度** | **10 个族只有 1 个 validated** | 每个 validated 族 = 一批采用。但广度依赖 P0-信任(先有可信曲线方法论)。**注意:不要陷入"验证全部 10 个"的跑步机**——先钉 3 条(每范式一条:image-diffusion / video-diffusion / AR-image)作为信任锚。 |
| **P1 正确性** | 持续硬化 replay/log-prob 契约 + E2E 不在 CI | 这是护城河本身。convergence 回归现在 CI 看不见(GPU 训练不在 PR CI)。 |
| **P1 上手** | 依赖碎片化(11 个 extra、`ar-vllm` 钉旧 torch 冲突) | "装哪些 extra 跑哪个用例"没指引;用户装错或装太多。 |
| **P1 广度** | 加新模型族无文档/模板 | 护城河#2 的价值要靠"别人能轻松加族"释放;现在 registry 无 docstring、无 integration guide。 |
...
### 阶段 B —— 上手层(P0)
- **`README` quickstart**:一条命令跑通最小真实 RL job(SD3.5-OCR),写清"2 分钟内你该看到什么"。
- **依赖矩阵**:把 11 个 extra 映射到用例("训 SD3.5 装 X;Wan video 装 Y");修 `ar-vllm` torch 冲突。
- **"Why visual-rl, not slime/verl"** 一页:把 §1 的结构性失配讲给外部看。

### 阶段 C —— 广度层(P0/P1)
- **"Adding a Model Family" 指南 + 模板**:释放护城河#2。
...
3. **加族成本**:加一个新 diffusion/AR 族到能跑 RL < 1 天,只动注册表 + 族目录,不动 trainer/algorithm。(今天:接近,但无文档)
...
**TL;DR**:别做 vLLM 的推理服务克隆。做**视觉生成式 RL 的 DeepSpeed**——押"这是个独立的正确性+系统问题"。护城河(去噪 rollout / 统一族契约 / diffusion RL 数学)已经有了;差的是**让人信(可复现曲线)、让人用(quickstart)、让人扩(加族指南)**。性能别当卖点(已近 MFU 顶),正确性和覆盖才是。
